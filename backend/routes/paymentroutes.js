...
router.post("/payments/create", verifyToken, authorizeUserTypes("admin", "member"), authorizeRoles("admin", "staff", "member"), PaymentController.createMembershipPayment);

router.post("/webhooks/xendit", verifyXenditWebhook, PaymentController.xenditWebhook);

router.get("/admin/payments", verifyToken, authorizeUserTypes("admin"), authorizeRoles("admin", "staff", "superadmin"), PaymentController.getAllPayment);
