...
class DiscountRequestController {
    async createDiscountRequest(req, res, next) {
        try {
            const result = await DiscountRequestService.createDiscountRequest(req.auditMeta, req.fileUrls, req.user);
    ...
    async payAsRegularInstead(req, res, next) {
        try {
            const result = await DiscountRequestService.payAsRegularInstead(req.auditMeta, req.user);
            return res.status(200).json(result);

        } catch (error) {
            debuggerLog("payAsRegularInstead Controller", error);
            next(error);
        }
    }
    ...
    async findDiscountRequestByClientId(req, res, next) {
        try {
            const result = await DiscountRequestService.findDiscountRequestByClientId(req.user.id);

            return res.status(200).json(result);
        } catch (error) {
            debuggerLog("findDiscountRequestByClientId Controller: " + error.message);
            next(error);
        }
    }
